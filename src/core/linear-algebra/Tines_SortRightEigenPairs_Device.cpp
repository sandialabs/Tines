...
namespace Tines {
  ...
  template<typename RealType>
  int SortRightEigenPairsDeviceHIP
  (const Kokkos::HIP &exec_instance,
   ...
   const control_type &control) {
    ProfilingRegionScope region("Tines::SortRightEigenPairsHIP");
  ...
  int SortRightEigenPairsDevice<Kokkos::HIP>::
  invoke(const Kokkos::HIP &exec_instance,
   ...
   const control_type &control) {
    return SortRightEigenPairsDeviceHIP
      (exec_instance, er, ei, V, W, control);
  }
