#ifndef __TINES_APPLY_Q_INTERNAL_HPP__
#define __TINES_APPLY_Q_INTERNAL_HPP__
...
namespace Tines {
  ...
  struct ApplyQ_LeftForwardInternal {
    ...
    invoke(const MemberType &member, const int m, const int n, const int k,
      ...
      for (int m_A0 = (k - 1); m_A0 >= 0; --m_A0) {
        ...
        ApplyLeftHouseholderInternal::invoke(member, m_A2, n, tau,
                                             A_part3x3.A21, as0, B_part3x1.A1,
  ...
  struct ApplyQ_LeftBackwardInternal {
    template <typename MemberType, typename ValueType>
    KOKKOS_INLINE_FUNCTION static int
    invoke(const MemberType &member, const int m, const int n, const int k,
           /* */ ValueType *A, const int as0, const int as1,
  ...
  struct ApplyQ_RightForwardInternal {
    template <typename MemberType, typename ValueType>
    KOKKOS_INLINE_FUNCTION static int
    invoke(const MemberType &member, const int m, const int n, const int k,
           /* */ ValueType *A, const int as0, const int as1,
      ...
      for (int n_A0 = 0; n_A0 < k; ++n_A0) {
        ...
        ApplyRightHouseholderInternal::invoke(member, m, n_B2, tau,
                                              A_part3x3.A21, as0, B_part1x3.A1,
                                              bs0, B_part1x3.A2, bs0, bs1, w);
        member.team_barrier();
        /// -----------------------------------------------------
        A_part2x2.mergeToATL(A_part3x3);
        t_part2x1.mergeToAT(t_part3x1);
