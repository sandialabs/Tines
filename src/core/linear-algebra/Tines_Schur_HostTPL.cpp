...
extern "C" void slahqr_(const int *wantt, const int *wantz, const int *n,
                        const int *ilo, const int *ihi, float *H,
                        const int *ldh, float *wr, float *wi, const int *iloz,
                        const int *ihiz, float *Z, const int *ldz, int *info);
...
namespace Tines {
  ...
  int Schur_HostTPL(const int m, float *H, const int hs0, const int hs1,
    ...
    const int ilo(1), ihi(m), iloz(1), ihiz(m);
    const int ldh = hs1, ldz = zs1;
    slahqr_(&wantt, &wantz, &m, &ilo, &ihi, H, &ldh, er, ei, &iloz, &ihiz, Z,
            &ldz, &r_val);
