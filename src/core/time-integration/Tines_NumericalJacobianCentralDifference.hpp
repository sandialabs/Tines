#ifndef __TINES_NUMERICAL_JACOBIAN_CENTRAL_DIFFERENCE_HPP__
#define __TINES_NUMERICAL_JACOBIAN_CENTRAL_DIFFERENCE_HPP__
...
namespace Tines {
  ...
  struct NumericalJacobianCentralDifference {
    ...
    invoke(const MemberType &member,
      ...
      const real_type zero(0), /*one(1),*/ half(0.5), two(2);
      ...
      for (int j = 0; j < m; ++j) {
        ...
        Kokkos::parallel_for(
          Kokkos::TeamVectorRange(member, m),
          [&](const int &i) { J(i, j) = half * (f_h(i) - f_0(i)) / h; });
