...
namespace Tines {
  ...
  template <typename SpT> struct HessenbergDevice {
    ...
    static int invoke(
      ...
      const value_type_3d_view<float, typename UseThisDevice<SpT>::type> &Q,
      const value_type_2d_view<float, typename UseThisDevice<SpT>::type> &t,
  ...
  template <> struct HessenbergDevice<Kokkos::HIP> {
    static int invoke(
