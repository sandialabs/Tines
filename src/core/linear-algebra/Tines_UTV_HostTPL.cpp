...
namespace Tines {
  ...
  defined(TINES_ENABLE_TPL_CBLAS_ON_HOST)
    ...
    /// factorize qr with column pivoting
    r_val = LAPACKE_sgeqp3(layout, m, n, (float *)A, lda, (int *)jpiv,
                           (float *)tau);

    /// find the matrix matrix_rank
    {
      const int as = as0 + as1;
      float max_diagonal_A(std::abs(A[0]));
      for (int i = 1; i < min_mn; ++i)
        max_diagonal_A = std::max(max_diagonal_A, std::abs(A[i * as]));

      const float eps = ats<float>::epsilon();
      const float threshold(max_diagonal_A * eps);
      matrix_rank = min_mn;
      for (int i = 0; i < min_mn; ++i) {
        if (std::abs(A[i * as]) < threshold) {
          matrix_rank = i;
    ...
    r_val = LAPACKE_sorgqr(layout, m, m, matrix_rank, (float *)U, ldu,
                           (float *)tau);
    ...
    if (matrix_rank < min_mn) {
      ...
      /// qr on the transpose R
      r_val = LAPACKE_sgeqrf(layout_transpose, n, matrix_rank, (float *)A, lda,
                             (float *)tau);
      ...
      /// form V with transposed layout
      r_val = LAPACKE_sorgqr(layout_transpose, n, n, matrix_rank, (float *)V,
                             ldv, (float *)tau);
    }
    return r_val;
#else
